...
class Setting {
	...
	public:
		struct {
			...
			const int BUTTON_AIR_RECIRCULATION_PIN = 10;         	// Кнопка рециркуляции // pinMode(buttonPin, INPUT_PULLUP);
			const int BUTTON_AIR_CONDITIONING_PIN = 11;          	// Кнопка кондиционера // pinMode(buttonPin, INPUT_PULLUP);
			const int SOFTWARE_TX = 12								// Программный TX, Если нужно поменять
			const int SOFTWARE_RX = 13								// Программный RX
												//Аналоговые
			const int HEATER_TEMP_PIN = A0;                      	// A0 = Температура в печке
		} pin;
		
		struct {
			byte mode = 1;									// Режим работы (0 - серсисный, 1 - нормальный)
